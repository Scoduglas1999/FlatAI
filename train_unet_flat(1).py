...
def variable_size_collate(batch):
    """
    Custom collate_fn that handles batches of images with varying sizes.
    It returns lists of tensors instead of a single stacked tensor.
    """
    affected_list = [item[0] for item in batch]
    sharp_list = [item[1] for item in batch]
    flat_list = [item[2] for item in batch]
    grad_list = [item[3] for item in batch]
    return affected_list, sharp_list, flat_list, grad_list
...
def main():
    ...
    # Data
    train_dataset = FlatFieldDataset(root_dir=DATA_DIR, mode='train', epoch_size=10000)
    val_dataset = FlatFieldDataset(root_dir=DATA_DIR, mode='val')
    
    # --- CRITICAL FIX for MEMORY OVERLOAD ---
    # Setting num_workers=0 forces data loading to happen serially in the main process.
    # This is slower but avoids multiprocessing memory issues with very large images.
    num_workers = 0
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        for i, (affected_list, sharp_list, flat_list, grad_list) in enumerate(pbar):
            optimizer.zero_grad(set_to_none=True)
            
            # Accumulate loss over the batch
            batch_loss = 0.0
            
            for affected, sharp, flat, grad in zip(affected_list, sharp_list, flat_list, grad_list):
                # Each item is a single image, add a batch dimension
                affected = affected.unsqueeze(0).to(device)
                ...
                batch_loss += total_sample_loss / len(affected_list)

            # --- Backward pass after processing all samples in the batch ---
            if torch.is_tensor(batch_loss) and torch.isfinite(batch_loss):
                ...
                pbar.set_postfix_str(f"loss={batch_loss.item():.6f}")
        ...
        with torch.no_grad():
            ...
            for i, (affected_list, sharp_list, flat_list, grad_list) in enumerate(pbar_val):
                ...
                for affected, sharp, flat, grad in zip(affected_list, sharp_list, flat_list, grad_list):
                    ...
                    total_val_sample_loss = (LAMBDA_L1 * loss_l1 +
                    ...
                    batch_val_loss += total_val_sample_loss / len(affected_list)
                ...
                pbar_val.set_postfix_str(f"val_loss={batch_val_loss.item():.6f}")
